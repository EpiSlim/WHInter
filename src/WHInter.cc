...
private:
  // Data
  std::vector<std::vector<int>> Z;    // each element vector contains the non-zero elements of the corresponding column
  std::vector<std::vector<int>> Zinv; // each element vector contains the non-zero elements of the corresponding row
...
public:
  WHInter(std::string filename, int nlambda = 100, double lambdaMinRatio = 0.01,
          int maxSelectedFeatures = 150, bool useBias = 1, char useMyMips = 2,
          char typeBound = 2, int F = 50, double eps = 1e-8) {
  ...
  void pre_solve(int m, vector<int> &index, vector<double> &w,
                 vector<vector<int>> &x, vector<vector<int>> &key,
                 bool dual = false) {
  ...
  void solve() {
    for (int t = 0; t < args.nlambda; t++) {
      ...
      pre_solve(m, index, w, x, key, false); // condition on primal here
      clean_model(m, index, w, x, key);
      update_theta();
      ...
      for (int iterB = 1; iterB < 10000000; iterB++) {
        ...
        pre_solve(m, index, w, x, key, true); // condition on dual here
        clean_model(m, index, w, x, key);
        update_theta();
      }
  ...
  void clean_model(int m, vector<int> &index, vector<double> &w,
                   vector<vector<int>> &x, vector<vector<int>> &key) {
    mod.clear();
    for (int s = 0; s < m; s++) {
      int j = index[s];
